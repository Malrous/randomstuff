-- sdkapi-pubic.luarmor.net/library.lua
-- DEMINIFIED, OPTIMIZED AND RENAMED by hand
-- view https://github.com/Malrous/randomstuff/new/main/LuarmorStuff/sdkapi-library.lua for the unoptimized and renamed version that ensures 
...
local function L(M)
    ...
    O = game:GetService("HttpService"):JSONDecode(O.Body)
    local P = O.nodes
    ...
    return game:GetService("HttpService"):JSONDecode(O.Body)
end
local function U()
    G = tostring(G)
...
return setmetatable({}, {__index = function(W,X)
    local Y = a(X)
